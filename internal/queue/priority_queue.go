...
var (
	ErrOutOfCapacity = errors.New("zgtl: 超出最大容量限制")
	ErrEmptyQueue    = errors.New("zgtl: 队列为空")
)
...
func NewPriorityQueue[T any](capacity int, compare zgtl.Comparator[T]) *PriorityQueue[T] {
	sliceCap := capacity + 1
	if capacity < 1 {
		capacity = 0
		sliceCap = 64
