...
var (
	ErrRBTreeSameRBNode = errors.New("zgtl: RBTree不能添加重复节点Key")
	ErrRBTreeNotRBNode  = errors.New("zgtl: RBTree不存在节点Key")
)

type RBTree[K any, V any] struct {
	root    *rbNode[K, V]
	compare zgtl.Comparator[K]
	size    int
}
...
// NewRBTree 构建红黑树
func NewRBTree[K any, V any](compare zgtl.Comparator[K]) *RBTree[K, V] {
	return &RBTree[K, V]{
		compare: compare,
		root:    nil,
	}
